...
function destroy_database()
{
   ...
   if(! $retval )
   {
     die('Could not get data: create database' . mysql_error());
   }
   endDatabaseConnection($conn);
}
...
function relationUnassignCourse_Fac()
{
   ...
   if(!$retval )
   {
     die('Could not get data unassigncoursefac: ' . mysql_error());
      ...
      echo "Created UnassignCourse_Fac Table successfully";
   }
...
//populate_members and populate_members_mname and get_member_name and populate_course and get_fac_id and get_stud_id and get_admin_id has been changed kindly update it
